...
from models import db, Equipment, MaintenanceRecord  # models.py 파일에 정의된 db, Equipment, MaintenanceRecord
...
def index():
    ...
    next_month = start_of_month.replace(day=28) + timedelta(days=4)  # 다음 달 계산
    end_of_month = next_month - timedelta(days=next_month.day)  # 현재 달의 마지막 날 계산

    today_count = MaintenanceRecord.query.filter(MaintenanceRecord.maintenance_date == today).count()
    week_count = MaintenanceRecord.query.filter(MaintenanceRecord.maintenance_date.between(start_of_week, end_of_week)).count()
    month_count = MaintenanceRecord.query.filter(MaintenanceRecord.maintenance_date.between(start_of_month, end_of_month)).count()
...
def maintenance_schedule():
    if request.method == 'POST':
        ...
        if record_id:
            ...
            record.maintenance_date = maintenance_date
            record.description = description
            flash('유지보수 일정이 수정되었습니다.', 'success')
        else:
            # 추가 요청 처리
            new_record = MaintenanceRecord(
                ...
                maintenance_date=maintenance_date,
                description=description,
                cost=0  # 비용이 필요하지 않다면 0으로 설정하거나 다른 방식으로 처리할 수 있습니다.
            )
            db.session.add(new_record)
            flash('유지보수 일정이 추가되었습니다.', 'success')
...
@app.route('/delete_schedule/<int:record_id>', methods=['POST'])
def delete_schedule(record_id):
    record = MaintenanceRecord.query.get_or_404(record_id)
    db.session.delete(record)
    db.session.commit()
    flash('유지보수 일정이 삭제되었습니다.', 'success')
    return redirect(url_for('maintenance_schedule'))
